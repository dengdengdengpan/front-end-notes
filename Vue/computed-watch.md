...
**`computed` 是计算属性，它会基于所依赖的响应式 property 动态显示最新的计算结果；计算属性的结果会被缓存，只有其依赖的响应式 property 有变化才会重新计算**。尽管计算属性形式上是方法，但它最终会被混入到 Vue 实例中当作 property 使用，所有 getter 和 setter 的 this 上下文自动地绑定为 Vue 实例，用法示例如下：
...
要实现的需求是，当点击全部按钮时，展示所有的用户列表；当点击男按钮时，展示性别为男的用户列表；当点击女按钮时，展示性别为女的用户列表。
...
- 通过计算属性实现时，在计算属性 `displayUsers` 中添加一个 log：
  ...
  **计算属性 `displayUsers` 会根据依赖的 `gender` property 进行计算，并且会缓存计算结果**。当点击不同按钮时，`gender` 发生了改变，`displayUsers` 会马上重新求值；当连续点击相同按钮时，`gender` 没有发生改变，`displayUsers` 会立即返回之前的计算结果，而不会重新求值。
  ...
  需要缓存的好处在于，如果有一个场景需要进行大量计算，页面开销性能较大。使用计算属性则可以利用缓存减少页面开销；使用方法会每次都重新计算，从而可能会影响用户体验。
...
new Vue({
  ...
  // 需观察的 data 中的 property 作为 watch 的 key
  ...
  watch: {
    ...
    // 第二种，值为方法名
    b: 'methodName',
    // 第三种，值为包含选项的对象，选项有：deep、immediate
    c: {
...
`watch` 会在其观察的 property 的值发生变化时作出响应，接下来了解一下 property 值的变化：

```html
<div id="app">
  <button @click="n += 1">n + 1</button>
  <button @click="visible = !visible">!visible</button>
  <button @click="user.nickname = '娜美'">user.nickname = '娜美'</button>
  <button @click="user = { nickname: '路飞' }">user = 新对象</button>
</div>
<script>
new Vue({
  el: '#app',
