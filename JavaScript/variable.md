...
因为 `var` 声明的变量会在执行任何代码前先进行处理，所以在代码中任何位置声明变量都等同于在当前作用域顶部声明它。 这意味着可以在声明变量前就使用它，该行为被称为**变量提升**，即 `var` 声明的变量像是被"移动"到当前作用域的顶部一样。
...
这导致 **`var` 声明的变量会在任何代码被执行前先被编译器处理**。对于 `var name = 'lufei';` 语句，JavaScript 会将其看作两个部分：
...
1. 第一部分在**编译阶段**进行，变量被创建，并且允许在声明变量之前访问，此时变量的值为 `undefined`。
2. 第二部分则被留在原地等待**执行阶段**，变量的初始化也只有在到达赋值语句时才会进行。

总之，**只有声明本身会被提升，赋值或其它运行逻辑都会留在原地**。因此，”提升“实际是指 `var` 声明部分在编译阶段被提前处理，从而产生变量可以在未声明前就能使用的现象。
...
```javascript
function fn() {
  var name; // 将 var 声明提升到当前函数作用域的顶部
